...
VitalServiceJson = function(logger, loggingEnabled) {
  ...
  // console.log("HHHHHHEEEEELLLLLL OOOOOOO!")
...
// console.log("domains.length", domains);
	...
	for(var i = domains.length -1; i >= 0; i--) {
      ...
      // console.log("roots is:", roots); // roots is an array

		} else {
			domain.parentsCopy = domain.parents.slice();
      // console.log("domain.parentsCopy", domain.parentsCopy)
		}
	}
	...
	while(roots.length > 0) {
		...
		out.push(removed);
    // console.log("out is:", out.length); // out is an array
		var uri = removed.domainURI;
    // console.log("URI", uri);
		for(var i = domains.length - 1; i >= 0; i--) {

			var d = domains[i];
      // console.log("domains[i]", d);
  ...
  console.log("copy.lnegth", copy.length);
